...
% 1~10 class
table = [-100,-3.4,0.5361,-100,-2.8,0.2216,-100,-2.6,0.1469,-100,-2.7,0.1447,-100,-2.8,0.2302; ...
...
8.9,100,5.7642,12.2,100,16.3659,14.8,100,33.0952,16.4,100,57.7500,17.5,100,100.00];
%%
% get ap for each RRI
for i = 1:length(ap)
    PrP = 0.04;
    PrO = PrP/(1-PrP);
    for j = 1:5
        id = find(table(:,1+(j-1)*3) <= RRI_diff(i+(j-1)) & [table(2:end,1+(j-1)*3);100] > RRI_diff(i+(j-1)));       
        id = id(1);
