...
for kk = 1: length(c)
	idx = max(1,c(kk)-RR): min(length(tfrsqtic),c(kk)+RR) ; %Range to accumulate energy
	recon(kk) = 2*sum(tfrsq(idx,kk),1)*(Hz/2/0.5)*(alpha)/Hz ; %why2 * sum_of_energy * sth * \Delta f
end
