...
len_buffer = 2*round(1/tempo(1)*osr);
onset = [zeros(1,len_buffer) onset];
tempo = [ones(1,len_buffer) tempo];
...
for i = (len_buffer+1):length(localscore)%-100
    ...
    if timerange(1) < 1
        ...
        % Also cut txwt to have the same length as timerange
        txwt = txwt(timerange>0);
    end
